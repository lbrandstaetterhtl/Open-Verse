...
async function runAdvancedAttack() {
    ...
    setTimeout(() => {
        ...
        console.log(`- Status: ${RESULTS.dos.blocked > 0 ? "✅ Blocked (413 Payload Too Large)" : "⚠️ Accepted/Parsed (Resource Drain Risk)"}`);
        if (RESULTS.dos.success > 0) console.log("  -> Recommendation: Configure 'express.json({ limit: ... })'");

        console.log("\nVECTOR 2: PARAMETER POLLUTION");
        console.log(`- Status: ${RESULTS.pollution.error === 0 ? "✅ Handled Gracefully" : "❌ Server Error (Logic Break)"}`);
