...
async function main() {
    try {
        ...
        if (report.ipAddress === testIp) {
            ...
            console.error(`FAILURE: Returned IP address mismatch. Expected ${testIp}, got ${report.ipAddress}`);
        }
        ...
        if (fetchedReport) {
            if (fetchedReport.ipAddress === testIp) {
                ...
                console.error(`FAILURE: Database IP address mismatch. Expected ${testIp}, got ${fetchedReport.ipAddress}`);
            }
        } else {
