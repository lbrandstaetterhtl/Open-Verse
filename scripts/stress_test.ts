...
function attackLogin() {
    ...
    const req = http.request({
        hostname: HOST, port: PORT, path: '/api/login', method: 'POST',
...
--${boundary}--`;
    ...
    const req = http.request({
        hostname: HOST, port: PORT, path: '/api/posts', method: 'POST',
        headers: {
        ...
        if (res.statusCode === 401) STATS.upload.blocked++; // Auth blocked
        else if (res.statusCode === 500) STATS.upload.blocked++; // Likely our Filter Error threw exception (handled by express usually as 500)
        else if (res.statusCode === 201) STATS.upload.success++; // BAD!
...
function generateTraffic() {
    ...
    const req = http.request({
        hostname: HOST, port: PORT, path: '/api/posts', method: 'GET'
    }, (res) => {
        STATS.traffic.sent++;
...
async function runStressTest() {
    ...
    const printer = setInterval(() => {
        process.stdout.write(`\r[⏳] Status: Logins(${STATS.login.blocked} blocked) | Uploads(${STATS.upload.sent}) | Traffic(${STATS.traffic.success} OK)   `);
    }, 1000);
    ...
    setTimeout(() => {
        ...
        console.log(STATS.login.blocked > 0 ? "✅ Rate limit IS WORKING." : "❌ Rate limit NOT DETECTED.");
        ...
        console.log(STATS.upload.success === 0 ? "✅ Uploads PROTECTED (Auth/Filter)." : "❌ Malicious file ACCEPTED.");
        ...
        console.log(STATS.traffic.error === 0 ? "✅ Server REMAINED STABLE." : "⚠️ Server showed instability.");
